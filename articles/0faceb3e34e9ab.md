...
| MultiNLI | contradictory / entailment | no negation / negation | NLI ラベル $\times$ 否定語の有無 |
...
### 4.3 正則化は Group DRO の補助ではなく、成立条件である
...
したがって、正則化は Group DRO の単なる補助ではない。過剰パラメータ化モデルにおいて、Group DRO を worst-group generalization に結びつけるための中心的な **成立条件** である。
