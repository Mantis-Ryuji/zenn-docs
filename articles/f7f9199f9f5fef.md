...
$\mathbf{V}$ の列ベクトルの長さは 1 であるから、 $\mathbf{S}$ の固有ベクトルは $\mathbf{V}$ の列であり、固有値は
...
固有値 $\lambda_m$ は特異値 $\sigma_m$ の2乗に比例する。特異値が大きい順に並んでいるということは、固有値も自動的に大きい順に並んでいることになる。
