...
topics: ["AI", "機械学習", "線形代数", "Python", "PyTorch", "次元削減"]
published: false
---
...
2次元テンソル `X (N, d)` を **SVDベースPCA**で `k=n_components` 次元へ射影し、
...
を取得する。
この実装は `torch.linalg.svd` を用いるため、**GPUテンソルを渡せばGPU上で計算**される。


#### 2. 基本使用例（fit → transform）

以下で実装した pca.py を同階層に置く前提：
...
W = pca.loadings              # (256, 32)  主軸（V_k）
mu = pca.mean                 # (256,)     平均
ev = pca.explained_variance_  # (32,)      λ_i = σ_i^2 / N
```

* `center=True` のとき `X` を特徴量平均で中心化してからPCAする。
* `loadings`（= `loadings_`）は `(d,k)` の列正規直交ベクトル（`V_k`）。
* `Z = (X - mean) @ loadings` が transform の中身。

#### 3. fit_transform（まとめて実行）

```python
Z = PCA(n_components=32).fit_transform(X)
```

`fit()` の後に同じ `X` を `transform()` するだけ。

#### 4. whitening（任意）

`whiten=True` にすると、transform後に各成分を `sqrt(explained_variance_)` で割る：
...
* `save()` は学習済みパラメータをCPUへ落として `.pt` に保存する。
* `load()` は `map_location` で読み込み位置を決め、`device` 指定があれば最終的にそこへ移す（優先）。

#### 6. 主要属性（学習後に参照可能）

学習後、以下にアクセスできる：

* `pca.mean_` / `pca.mean`：特徴量平均 `(d,)`
* `pca.loadings_` / `pca.loadings`：主軸 `(d,k)`
* `pca.components_`：`loadings_` の別名（sklearn互換）
* `pca.singular_values_`：特異値 `(k,)`
* `pca.explained_variance_`：分散 `(k,)`（この実装は **σ^2 / N**）
* `pca.explained_variance_ratio_`：寄与率 `(k,)`（**上位k成分内で正規化**）
* `pca.n_samples_`：学習サンプル数
* `pca.fitted_`：学習済みフラグ

#### 7. 入力制約（Fail Fast）

* `X` は **torch.Tensor かつ float**（整数は不可）
* `X.ndim == 2`（形状は `(N, d)`）
* `transform()` の `d` は fit時と一致必須
  これらに違反すると `TypeError` / `ValueError` で即落ちる。
