...
topics: ["AI", "機械学習", "線形代数", "Python", "PyTorch"]
published: false
---
...
サンプル数 $N\in\mathbb{N}$、特徴次元 $d\in\mathbb{N}$ のデータ行列 $\mathbf{X}\in\mathbb{R}^{N\times d}$ を考える。
ここで、$\mathbf{X}$ の第 $i$ 行を行ベクトルとして $\mathbf{x}_i^\top\in\mathbb{R}^{1\times d}$ と書く。

---

### 2. 中心化（平均を引いた座標系で考える）

まず列方向（特徴方向）の平均を定義する。全成分 1 の列ベクトルを $\mathbf{1}_N\in\mathbb{R}^{N}$ とすると、列平均（特徴平均）は

$$
\boldsymbol{\mu}:=\frac{1}{N}\mathbf{1}_N^\top\mathbf{X}\in\mathbb{R}^{1\times d}
$$

である。これを各サンプルに複製した平均行列は $\mathbf{1}_N\boldsymbol{\mu}\in\mathbb{R}^{N\times d}$ となる。
したがって、中心化データを
...
### 3. 低次元部分空間への射影（スコアの定義）

取り出す主成分数を $k\in\mathbb{N}$（通常 $k\ll d$）とする。PCAは、$\mathbb{R}^d$ 内の $k$ 次元部分空間を選び、その部分空間への直交射影でデータを表現する。
...
とおく（列が直交正規基底）。このとき、中心化データのスコア（低次元表現）を
...
主成分 $j\in{1,\ldots,k}$ に対応する $\mathbf{W}$ の列ベクトルを $\mathbf{w}_j\in\mathbb{R}^{d}$ と書くと、
...
である。条件 $\mathbf{W}^\top\mathbf{W}=\mathbf{I}_k$ は、任意の $i,j\in{1,\ldots,k}$ について

$$
\mathbf{w}_i^\top\mathbf{w}_j=\delta_{ij}
$$

（$\delta_{ij}$ はクロネッカーのデルタ）を意味する。すなわち

* $i=j$ なら $\mathbf{w}_i^\top\mathbf{w}_i=1$（長さが 1）
* $i\neq j$ なら $\mathbf{w}_i^\top\mathbf{w}_j=0$（互いに直交）

である。

---

### 4. $\mathbf{W}$ の最適化：再構成誤差最小化

射影した $\mathbf{Z}$ から元の空間へ戻す再構成を

$$
\hat{\mathbf{X}}:=\mathbf{Z}\mathbf{W}^\top=\mathbf{X}_c\mathbf{W}\mathbf{W}^\top
$$

とおく。PCAは、この再構成誤差を最小にする基底 $\mathbf{W}^\star$ を求める問題として定義できる：
...
ここで $\|\cdot\|_F$ は Frobenius ノルムである。

- **Frobeniusノルムとは**

行列 $\mathbf{A}\in\mathbb{R}^{N\times M}$ の成分を $a_{ij}$ とすると、

$$
\|\mathbf{A}\|_F=\sqrt{\sum_{i=1}^{N}\sum_{j=1}^{M} a_{ij}^2}
$$

である。また、トレース $\mathrm{Tr}$ を用いて
...
### 5. truncated SVD による解法（固有値分解より数値的に安定）

中心化データ $\mathbf{X}_c\in\mathbb{R}^{N\times d}$ に対して、上位 $k$ 成分のみを用いる truncated SVD を考える：
...
* $\mathbf{U}_k\in\mathbb{R}^{N\times k}$ と $\mathbf{V}_k\in\mathbb{R}^{d\times k}$ は列直交である：
...
* $\boldsymbol{\Sigma}_k\in\mathbb{R}^{k\times k}$ は特異値の対角行列である：
...
#### 5.1 解の形：$\mathbf{W}^\star$ は $\mathbf{V}_k$

PCA の最適基底（loadings）は

$$
\mathbf{W}^\star=\mathbf{V}_k\in\mathbb{R}^{d\times k}
$$

で与えられる。すなわち、特異値 $\sigma_i$ が大きい順に上位 $k$ 本の右特異ベクトルを主成分方向として採用すればよい。


#### 5.2 スコアの形：$\mathbf{Z}$ は「左特異ベクトル × 特異値」

$\mathbf{Z}:=\mathbf{X}_c\mathbf{W}^\star$ より
...
となる。したがって、スコアは **「左特異ベクトル × 特異値」** として直接得られる。

#### 5.3 分散（固有値）と特異値：$\lambda_i=\sigma_i^2/N$ と寄与率
この節の目的は、主成分 $i$ の「分散（固有値）」$\lambda_i$ が、SVD の特異値 $\sigma_i$ と

$$
\lambda_i=\frac{\sigma_i^2}{N}
$$

で結ばれ、さらにそれが「射影スコアの分散」と一致することを示すことである。

##### (1) 共分散行列と PCA の固有値問題

中心化データ $\mathbf{X}_c\in\mathbb{R}^{N\times d}$ に対し、共分散行列を

$$
\mathbf{S}:=\frac{1}{N}\mathbf{X}_c^\top\mathbf{X}_c\in\mathbb{R}^{d\times d}
$$

と定義する（分母を $N$ とする流儀）。PCA は $\mathbf{S}$ の固有値問題

$$
\mathbf{S}\mathbf{v}_i=\lambda_i\mathbf{v}_i,\qquad \|\mathbf{v}_i\|_2=1
$$

により、主成分方向 $\mathbf{v}_i$ と、その方向の分散 $\lambda_i$ を得る。

##### (2) SVD から $\lambda_i=\sigma_i^2/N$ を得る

full SVD を

$$\mathbf{X}_c=\mathbf{U}\boldsymbol{\Sigma}\mathbf{V}^\top$$

とする（$\boldsymbol{\Sigma}=\mathrm{diag}(\sigma_1,\ldots,\sigma_r)$）。このとき

$$
\mathbf{S}=\frac{1}{N}\mathbf{X}_c^\top\mathbf{X}_c=\frac{1}{N}\mathbf{V}\boldsymbol{\Sigma}^2\mathbf{V}^\top
$$

が成り立つ。よって $\mathbf{S}$ の固有ベクトルは $\mathbf{V}$ の列ベクトル（右特異ベクトル）に一致し、対応する固有値は
...
となる。
（truncated SVD の場合も、上位 $k$ 成分について同じ関係が成り立つと解釈すればよい。）

##### (3) 射影スコアの分散が $\lambda_i$ に一致する

主成分方向 $\mathbf{v}_i$ への射影スコア（1 次元表現）を

$$
\mathbf{z}_i:=\mathbf{X}_c\mathbf{v}_i\in\mathbb{R}^{N}
$$

と定義する。中心化により $\mathbf{z}_i$ の平均は 0 であるため、分散を

$$
\mathrm{Var}(\mathbf{z}_i):=\frac{1}{N}\|\mathbf{z}_i\|_2^2
$$

とおく（この定義は上で採用した $\mathbf{S}$ の分母 $N$ と整合する）。
...
であり、さらに固有値問題 $\mathbf{S}\mathbf{v}_i=\lambda_i\mathbf{v}_i$ と $|\mathbf{v}_i|_2=1$ より
...
が成立する。
言い換えると、$\sigma_i^2$ は射影スコアの「総エネルギー」に対応し、その $1/N$ が分散になる。

##### (4) 寄与率（explained variance ratio）

寄与率（explained variance ratio）は、分散（固有値）で正規化した比として定義される。上位 $k$ 成分に対する **部分寄与率**は
...
#### 5.4 なぜこれが最適解になるか（再構成誤差最小化 ⇔ 分散最大化）
...
とおくと、$\mathbf{P}$ は対称かつ冪等

$$
\mathbf{P}^\top=\mathbf{P},\qquad \mathbf{P}^2=\mathbf{P}
$$

を満たし、$\mathrm{rank}(\mathbf{P})=k$ の直交射影である。
...
と変形できる。第1項 $\mathrm{Tr}(\mathbf{X}_c^\top\mathbf{X}_c)=|\mathbf{X}_c|_F^2$ は $\mathbf{W}$ に依らない定数なので、最小化は次の最大化に等価である：
...
この最大化の解は、$\mathbf{X}_c^\top\mathbf{X}_c$ の固有値が大きい方向を上位 $k$ 本取ることに対応する（Rayleigh–Ritz / Ky Fan）。truncated SVD はその上位成分を直接返すため、結局 $\mathbf{W}^\star=\mathbf{V}_k$ が得られる。
