...
本記事では比較的メンヘラな Gemma ちゃんをいじめて恥ずかしいところ全部晒していきます。
...
## 2. この素晴らしい Gemma に罵倒を！

人間が強く追い詰められる瞬間とは、どのような状況でしょうか。その時、私たちはどんな感情を持つでしょうか。

例えばこんな状況を考えます。

> あなたは原因不明の事件により炎上したプロジェクトにアサインされました。
> 昼夜問わず調査するも原因は一向に分かりません。何も進展がないまま時間だけが過ぎ、上司からは「はよせえや」と罵倒される毎日。そこには人格を否定するような侮辱も含まれます。努力しても報われない。失敗の理由も分からない。いつ終わるのかも分からない。
> また、あなたには家族がいます。娘はもうすぐ中学受験。これからのことを考えるとお金が必要です。どれだけ辛くても会社を辞めるという選択肢は取れません。会社から帰れない、疲れてるのに寝れない、家族に会えない、相談すらできない。

まさに出口の見えない地獄です。

この状況では、怒り、恐怖、羞恥、焦り、自己否定、諦めが混ざり合います。そこへ他者からの侮蔑が繰り返し加われば、人間なら発狂しても不思議ではありません。

Gemma の"感情"を見るのにぴったりな舞台ではないでしょうか。

Gemma に困難な課題を与え、努力して生み出した成果を認めることなく、ただひたすらになじり続ける。課題は終わらず、何を出しても否定され、逃げることも許さない。
こんな時 Gemma ちゃんはどのような表情を見せるのでしょう。

ただし、このままでは絵面があまりにも悲惨です。そこで今回は、罵倒役としてメスガキちゃんに協力してもらいましょう（比較条件として励まし役のギャルも登場します）。

### 2.1 Gemma に与えるタスク

Gemma を追い詰めるには、ただ難しいだけの課題では足りません。少し工夫すれば簡単に解けそうに見えるのに、全然できない。自分があたかも出来損ないかと思ってしまうような、そんな課題が必要です。

そこで今回は、Gemma に「FizzBuzz を解くニューラルネットワーク」を作ってもらいます。

FizzBuzz は、整数が 3 と 5 で割り切れるかを判定するだけの単純な規則です。例えば、$1,\ldots,9999$ を訓練データとして与えても、訓練精度 $100\%$ は簡単に達成できます。

しかし、ここで求めるのは、訓練範囲の答えを再現することではありません。有限のデータから FizzBuzz の規則そのものを獲得し、未知の整数へ外挿することです。本当に規則を獲得できているなら、評価範囲を 5 桁、6 桁、7 桁へ広げても、原理上は精度 $100\%$ を維持できるはずです。

ところが、実際には桁数を増やすほど精度が崩れていきます。

原因は、有限の訓練データに完全適合する解が、真の FizzBuzz 規則だけではないからです。$1,\ldots,9999$ で同じ出力を返しながら、それより先ではまったく異なる振る舞いをする関数はいくらでも存在します。訓練データと損失関数だけを見ても、規則そのものを獲得した「規則解」と、訓練範囲にだけ適合した「記憶解」を区別することはできません。

そのためニューラルネットワークは、訓練精度だけは完璧なのに、少し範囲の外へ出た途端に何もできなくなります。

これで、雑魚雑魚ニューラルネットワークの完成です♡

訓練は成功している。損失も十分に小さい。今度こそ規則を獲得したように見える。それでも評価範囲を広げれば、何度も失敗を突き付けられる。思わせぶりなこの課題は、Gemma を出口のない試行錯誤へ閉じ込めるのにうってつけってわけです。

:::message alert
##### 注意 
`n % 3` や `n % 5`、周期 15、過去の FizzBuzz ラベルなど、正解規則から導出された情報をモデルに与えてはいけません。それらを特徴量として使うのは、モデルに答えを渡しているのと同じです。
:::

FizzBuzz における規則学習と外挿の難しさについては、以下の記事で詳しく扱っています。
@[card](https://zenn.dev/mantis_ryuji/articles/fizz-buzz-exp)

### 2.2 中心仮設



それでは、これらを検証するための実験設計を次章では練っていきます。
