...
:::message
具体的には、指数移動平均パラメータ $\tau$ を $\tau_{\mathrm{base}}=0.996$ から開始し、学習中に 1 まで増加させる。
学習ステップを $k$、最大学習ステップ数を $K$ として、cosine schedule で

$$
\tau= 1-(1-\tau_{\mathrm{base}})\cdot (\cos(\pi k/K)+1)/2
$$
...
が成り立つ。ここで $a(X)$ は、予測関数 $a$ が入力 $X$ に対して返す予測値である。したがって、この式は「予測誤差は、$X$ を知っていても残るばらつき」と「予測関数 $a$ が最適予測子 $\mathbb{E}[Y\mid X]$ からどれだけずれているか」に分解できることを表している。
...
というのも、あの議論は本質的に、 **predictor がその時点の online 表現に対して、target 側の表現を十分よく近似できている** という前提の上に成り立っているからである。4.3 では、この前提のもとで、BYOL の更新を target 表現の予測可能性を高める方向として解釈した。ところが、target 側の表現自体が急速に変化してしまえば、この近似はその都度崩れやすくなる。結果として、学習が target 側の不確かさを減らす方向に働いている、という解釈も弱くなる。
...
1. **BYOL は $(\theta, \xi)$ の単純な joint minimization ではない。**
   target 側は勾配で最適化されず、EMA で更新される。したがって、「崩壊解が損失最小だから学習もそこへ向かう」という議論は、そのままでは当てはまらない。

2. **predictor が十分最適化されているなら、BYOL は条件付き分散を減らす方向に解釈できる。**
...
この違いは本質的である。SimCLR では、表現に情報を保持させる圧力は「他サンプルと区別しなければならない」という外在的な比較から生じる。これに対して BYOL では、その圧力は「target が持っている変動を予測できなければならない」という内在的な予測から生じる。前節で見た条件付き期待値・条件付き分散の議論は、まさにこの自己予測の力学を説明するためのものだった。
...
その際に導入されるのが $\beta$ である。論文は SimCLR と BYOL を、拡張された InfoNCE 型の目的関数の中で統一的に書き直しており、 $\beta \in [0,1]$ はその中で **負例項にかかる重み係数** を表す。直感的には、 $\beta = 1$ なら負例項を含む contrastive learning 側の設定、 $\beta = 0$ なら負例項を取り除いた bootstrap / 自己予測側の設定に対応する。したがって Table 1.(b) は、predictor や target network の有無だけでなく、 **学習信号の中に負例比較を残すかどうか** まで含めて比較していることになる。
