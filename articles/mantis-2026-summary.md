...
ドメイン汎化手法の論文は、流し読みも含めると有名どころは大体ざっと読んだが、結局は難しい数式をこねくり回すより、評価設計・データ設計・アノテーション設計をちゃんとやることの方が重要なのではないか、と感じている。

数式が無意味なのではなく、ドメイン汎化の実践では、数式で目的関数を工夫するだけでは足りず、どの分布差を評価し、どの失敗を重く見て、どの相関を壊すかという設計が本質になりやすいのだろう。そうした泥臭い設計判断が、最終的な OOD 性能を大きく左右する。
