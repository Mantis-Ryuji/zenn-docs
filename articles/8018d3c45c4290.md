...
![](https://storage.googleapis.com/zenn-user-upload/f062952d07c9-20260303.png =500x)
...
### 3.1. Augmentation の合成は良い表現学習に必要不可欠

Augmentation が表現学習に与える影響を体系的に評価するため、代表的な操作をいくつか取り上げて検討する。扱う Augmentation は大きく 2 種類に分類できる。

第一に、入力の空間配置そのものを変える **空間的/幾何学的変換** である。具体的には、ランダムクロップ＋リサイズ（水平反転を含む）、回転、カットアウトなどが該当する。

第二に、形状を保ったまま見え方を変える **外観の変換** である。例えば、色変換 (color dropping, brightness, contrast, saturation, hue) 、ガウシアンブラー、Sobel フィルタリングなどを扱う。

Fig. 3 は、以上の拡張操作の具体例を可視化したものである。

![](https://storage.googleapis.com/zenn-user-upload/d07fd856e4a9-20260303.png =500x)
*Fig.3 SimCLRで検討したAugmentationオペレータの例。各Augmentationは、内部パラメータ（例：回転角度、ノイズレベル）に基づいて確率的にデータを変換し得る。なお、これらのオペレータはアブレーション[^4]でのみ検証しており、モデル学習に用いるAugmentationには、ランダムクロップ（フリップおよびリサイズを含む）、色変換、ガウシアンブラーのみを含める。（元画像 cc-by: Von.grzanka）*

個々の Augmentation の寄与と、その合成が有効である理由を切り分けて理解するため、Augmentation を **単独** あるいは **2つの組み合わせ** で適用した場合の本手法の性能を評価する。ただし ImageNet は画像サイズが不揃いであるため、学習では常に **ランダムクロップ＋リサイズ** を適用する必要がある。この制約の下では、「クロップなし」という条件で他の拡張のみを比較することが難しく、クロップの影響が交絡しやすい。

そこで本アブレーションでは、この交絡を抑えるために **非対称なデータ変換** を採用する。具体的には、まず両ビューに対して常にランダムクロップを行い、同一解像度へリサイズする。そのうえで、検討対象の変換（単独または複数）を **Fig.2 の片側ブランチにのみ** 適用し、もう片側は恒等写像（すなわち $t(\mathbf{x}_i)=\mathbf{x}_i$ ）のままにする。

なお、この非対称な設定は一般に性能を低下させる。しかし、目的は絶対性能ではなく Augmentation 単体と合成がもたらす相対的な影響を観察することであり、その傾向自体はこの設定でも本質的に変わらないと考えられる。

![](https://storage.googleapis.com/zenn-user-upload/bda99aa3e9be-20260303.png =500x)
*Fig.4 片側のブランチにのみAugmentationを適用した場合における、個別のAugmentationおよびAugmentationの合成に対する線形評価（ImageNet Top-1 精度）。最後の列を除き、対角成分は単一の変換に対応し、非対角成分は2つの変換の合成（順に逐次適用）に対応する。最後の列は各行の平均を示す。*

Fig. 4 は、変換を **単独で適用した場合** と、**複数を合成して適用した場合** の線形評価結果を示す。コントラスト学習ではモデルが正例ペアをほぼ完全に識別できる状況であっても、**単一の変換だけでは良質な表現の獲得に不十分** であることを確認した。

一方、複数の拡張を合成するとコントラスト予測タスク自体は難しくなるが、その代償として **表現の品質は大きく向上** する。合成の中でも特に効果が顕著なのは、**ランダムクロップ** と **ランダムな色変換** の組み合わせであることが分る。

しかし、Augmentation としてランダムクロップのみを用いた場合の重大な問題として、**同一画像から切り出された多くのパッチが類似した色分布を共有してしまう** 点があると考えられる。実際、Fig. 5 が示すように **色ヒストグラムだけでも画像の識別が可能** であり、ニューラルネットワークがこのような近道に依存して予測タスクを解いてしまう恐れがある。

したがって、汎化可能な特徴を学習させるには、クロッピングを **色変換と組み合わせて** 色分布の一致という手掛かりを壊すことが重要である。

![](https://storage.googleapis.com/zenn-user-upload/72f58926e922-20260303.png =500x)
*Fig.5 2枚の異なる画像（2行）について、各画像から切り出した複数のクロップに対するピクセル強度（全チャネル）のヒストグラム。1行目の画像は Fig.3 から引用している。すべての軸は同一の範囲に揃えている。*

[^4]: 回転・ノイズ等の候補Augmentationを単体/組合せで試して効果を確認しただけで、最終的に本学習で使うAugmentation方針には全部は採用していないという意味
